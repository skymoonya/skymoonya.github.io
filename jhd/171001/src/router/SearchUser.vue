<template>
  <div class="root">
    ...
    <ul class="user-list">
      <li v-for="item in list" :key="item.id" @click="giveCard(item)">
...
export default {
  ...
  methods: {
    ...
    giveCard(user) {
      confirm({
        text: `是否将一张${this.cardName}赠送给${user.nickname}？`,
        okText: '立即赠送',
      }).then(() => {
        axios.post('service.activity_jhd.card.give', {
          recipientId: user.id,
          cardType: this.store.giveCardType,
          accountId: appSDK.param.accountId,
        }).then(({ data }) => {
          if (data.success) {
            tips('赠送成功')
  ...
  computed: {
    cardName() {
      switch (this.store.giveCardType) {
        case 1: return '天蓬卡'
        case 2: return '玉兔卡'
        case 3: return '后羿卡'
        case 4: return '玉帝卡'
